    ...
    public static class HarmonyPatches_ReservationSystem
    {
        ...
        public static void CanReserve_Postfix(ref bool __result, Pawn claimant, LocalTargetInfo target, int maxPawns, int stackCount, ReservationLayerDef layer, bool ignoreOtherReservations)
        {
                ...
                claimant?.CurJob?.def == PickUpAndHaulJobDefOf.UnloadYourHauledInventory)
            {
                ...
                // First evaluate existing PUAH reservations
                var claimantHasPuahRes = PUAHReservationSystem.PawnHasReservation(claimant, location, claimant.Map);
                var anyPuahRes        = PUAHReservationSystem.HasAnyReservation(location, claimant.Map);

                // If some other pawn (not the claimant) already has a PUAH reservation, block vanilla
                ...
                // If claimant already reserved via PUAH, always allow
                if (claimantHasPuahRes)
                {
                    __result = true;
                    return;
                }

                // Otherwise fall back to capacity-based check

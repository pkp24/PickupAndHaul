    ...
    public static class PUAHReservationSystem
    {
            ...
            out int availableCapacity)
        {
            ...
            if (bestCapacity > 0)
            {
                ...
                Log.Message($"PUAHReservationSystem found storage for {thing} at {locationStr} with capacity {bestCapacity} (NO RESERVATION MADE - will be reserved by JobDriver)");
                
                return true;
            }
            
            Log.Message($"PUAHReservationSystem found NO storage for {thing}");
            return false;
        }
